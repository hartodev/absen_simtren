 ...
 *
 *   TPQ (hijau, hanya pesantren)           -> pages/pesantren/dashboard-tpq
 *   grid modul (biru, sekolah / pondok)    -> pages/lembaga/dashboard
 */
class AdminDashboardController extends TenantAdminController
{
    public function show(Company $company)
    {
        if ($this->isTpq($company)) {
            return $this->view('dashboard-tpq', $this->tpqData($company));
        }
    ...
    private function tpqData(Company $company): array
    {
        $me    = Auth::user();
        $today = Carbon::today();
        $cid   = $company->id;
