...
<form method="POST" action="{{ route('superadmin.tenants.store') }}" class="mt-5 max-w-lg space-y-4 rounded-xl bg-white p-6 shadow-sm">
  ...
  <div>
    <label class="mb-1 block text-xs font-bold text-gray-500">Tampilan dashboard (pesantren / sekolah)</label>
    <select name="dashboard_style" class="h-12 w-full rounded-lg border border-gray-300 px-3">
      <option value="">Otomatis (sekolah = biru; pesantren tanpa asrama = hijau TPQ; dengan asrama = biru)</option>
      <option value="simple">Hijau — TPQ</option>
      <option value="full">Biru — sekolah umum / pondok pesantren</option>
    </select>
  </div>
  <label class="flex items-center gap-2 text-sm text-gray-600">
    <input type="checkbox" name="is_boarding" value="1"> Punya layanan asrama (pondok / sekolah pondok)
  </label>
