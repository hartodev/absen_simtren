...
<form method="POST" action="{{ url('/' . $prefix . '/employees/' . $karyawan->id) }}" class="mt-5 max-w-md space-y-4 rounded-xl bg-white p-6 shadow-sm">
  @csrf @method('PUT')
  <div>
    <label class="mb-1 block text-xs font-bold text-gray-500">Nama Lengkap</label>
    <input type="text" name="name" required value="{{ old('name', $karyawan->name) }}" class="h-12 w-full rounded-lg border border-gray-300 px-3">
  </div>
  <div>
    <label class="mb-1 block text-xs font-bold text-gray-500">Email (untuk login)</label>
    <input type="email" name="email" required value="{{ old('email', $karyawan->email) }}" class="h-12 w-full rounded-lg border border-gray-300 px-3">
  </div>
  <div>
    <label class="mb-1 block text-xs font-bold text-gray-500">Departemen</label>
    <input type="text" name="department" required value="{{ old('department', $karyawan->department) }}" class="h-12 w-full rounded-lg border border-gray-300 px-3">
  </div>
  <div>
    <label class="mb-1 block text-xs font-bold text-gray-500">Jabatan</label>
    <input type="text" name="position" required value="{{ old('position', $karyawan->position) }}" class="h-12 w-full rounded-lg border border-gray-300 px-3">
  </div>
