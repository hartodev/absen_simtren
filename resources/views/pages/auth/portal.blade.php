...
{{--
  Langkah 3 alur: Daftar -> Email aktivasi -> [HALAMAN INI: isi subdomain] -> Login subdomain.
  Ini BUKAN form email/password. Tugasnya cuma mengarahkan user ke
  halaman login milik lembaganya (subdomain).
--}}
  ...
  <div class="grid w-full max-w-4xl overflow-hidden rounded-3xl border border-[#dce6f3] bg-white shadow-[0_24px_70px_rgba(22,60,125,.12)] md:grid-cols-[.85fr_1.15fr]">
    ...
    <div class="hidden bg-[#102b69] p-10 text-white md:block">
      ...
      <div class="mt-24">
        ...
        <p class="mt-5 text-sm leading-6 text-blue-100/65">Masukkan subdomain lembaga Anda, lalu login di halaman lembaga tersebut.</p>
      </div>
      ...
      <form method="POST" action="{{ route('login.attempt') }}" class="mt-8 space-y-4">
        ...
        <label class="block">
          ...
          <div class="flex items-center overflow-hidden rounded-xl border border-[#cfd9e6] focus-within:border-[#2563eb]">
            <input type="text" name="subdomain" required autofocus placeholder="contoh: tpq-alhidayah"
                   class="h-12 w-full border-0 px-4 text-sm focus:ring-0"
        ...
        <button type="submit" class="button-lift mt-3 w-full rounded-xl bg-[#2563eb] py-3.5 text-sm font-bold text-white">
          Lanjut ke login <i data-lucide="arrow-right" style="width:15px;height:15px;margin-left:4px;display:inline"></i>
        </button>
      </form>

      <p class="mt-7 text-center text-xs text-[#8192aa]">
        Belum punya ruang institusi? <a href="{{ route('register.form') }}" class="font-bold text-[#2563eb]">Daftar sekarang</a>
      </p>
    </div>
  </div>
</main>
